...
function Overhead:OnLoad()
	...
	if IsAddOnLoaded("icicle") then self.db.enable = false log("Icicle is loaded, and it has a conflict with overhead, so overhead is disabled now. You can enable it again by type'/oh enable'") end
...
function Overhead:ResetAllNameplates()
	for nameplate in pairs(nameplates) do nameplate:ShowArt(); nameplate.shouldShowArt = false nameplates[nameplate] = nil end
end
